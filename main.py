...
app = FastAPI(title="TruthGuard AI v4.6")
templates = Jinja2Templates(directory="templates")
user_credits = {"free_user": 500} # Reset to 500
...
async def home(request: Request):
    ...
    if isinstance(credits, dict): credits = 500 # RESET IF BROKEN
    return templates.TemplateResponse("index.html", {"request": request, "credits": int(credits)})
...
async def process_data(request: Request, data: str = Form(None), file: UploadFile = File(None)):
    try:
        ...
        user_credits["free_user"] = int(credits) - credits_needed # FORCE INT
