...
PAYSTACK_PUBLIC_KEY = os.getenv("PAYSTACK_PUBLIC_KEY") # ADD THIS TO RAILWAY ENV
...
def NAVBAR(credits):
    # ADDED NAV LINKS
    return f"""<div style="background:#0d0d0d;padding:15px;border-bottom:2px solid #00ff88"><div style="max-width:900px;margin:0 auto;display:flex;justify-content:space-between;align-items:center"><a href="/" style="text-decoration:none"><b style="color:#00ff88">🛡️ TruthGuard AI</b></a><div><a href="/" style="color:#fff;margin-right:15px">Home</a><a href="/clean" style="color:#fff;margin-right:15px">Clean</a><a href="/pricing" style="color:#fff;margin-right:15px">Pricing</a>Credits: {credits}</div></div></div>"""
...
async def pricing(request: Request):
    session_id = get_session(request)
    # ADDED WORKING CHECKOUT BUTTON
    cards = "".join([f"<div style='border:2px solid #333;padding:20px;border-radius:15px;text-align:center'><h2>{p['name']}</h2><h1 style=color:#00ff88>₦{p['price']:,}</h1><p>{p['credits']:,} Credits</p><a href=/checkout/{k} class=btn>Buy Now</a></div>" for k,p in PLANS.items()])
...
# ADDED THIS ROUTE FOR PAYMENT
@app.get("/checkout/{plan_id}", response_class=HTMLResponse)
async def checkout(request: Request, plan_id: str):
    session_id = get_session(request)
    plan = PLANS.get(plan_id)
    if not plan: return HTMLResponse("Plan not found")
    amount = plan["price"] * 100 # kobo
    html = f"""<!DOCTYPE html><html><head><title>Checkout</title><script src="https://js.paystack.co/v1/inline.js"></script>{CSS}</head>
    <body>{NAVBAR(get_credits(session_id))}<div class=container style=text-align:center>
    <h1>Buy {plan['name']}</h1><p>₦{plan['price']:,} for {plan['credits']:,} Credits</p>
    <button onclick="pay()" class=btn>Pay With Paystack</button>
    <script>
    function pay(){{
      let handler = PaystackPop.setup({{
        ...
        callback: function(response){{
          window.location = '/verify?reference='+response.reference+'&plan={plan_id}';
        }}
      }});
      handler.open();
    }}
...
@app.get("/verify")
async def verify_payment(request: Request, reference: str, plan: str):
    # YOU NEED TO ADD PAYSTACK VERIFICATION HERE LATER
    session_id = request.cookies.get("tg_session")
    credits_to_add = PLANS[plan]["credits"]
    USERS[session_id] += credits_to_add
    save_users(USERS)
    return RedirectResponse(url="/pricing?status=success")
