...
# DATABASE SIMULATION - In real app use Railway Postgres
USERS = {} # {session_id: credits}

NEW_USER_CREDITS = 500
LIVE_PAYSTACK_KEY = "sk_live_YOUR_LIVE_KEY_HERE" # PUT YOUR LIVE KEY HERE

def get_credits(session_id):
    if session_id not in USERS:
        USERS[session_id] = NEW_USER_CREDITS # 500 for new customers
...
.shield{{font-size:4em}}
h1{{font-size:2.8em;margin:10px 0}}
p{{font-size:1.2em;color:#aaa;margin-bottom:30px}}
.btn{{background:#00ff88;color:#000;padding:16px 35px;border-radius:12px;text-decoration:none;font-weight:bold;margin:10px;display:inline-block;font-size:1.1em}}
...
<a href="/pricing" class="btn" style="background:#333;color:#fff">Buy Credits</a>
</div></body></html>"""
...
.shield{{font-size:2.5em;text-align:center}}
...
<p style="font-size:0.9em;color:#aaa">Secured by Paystack Live Key</p>
...
async def clean_page(request: Request):
    ...
    class_name = "error" if credits <= 0 else ""
    response = HTMLResponse(CLEAN_HTML.format(credits=credits, message=message, class=class_name, download_link="", disabled=disabled))
    response.set_cookie("session", session_id)
    return response
...
async def clean_data(request: Request, file: UploadFile = File(None), text_data: str = Form(None)):
    ...
    if credits <= 0:
        return HTMLResponse(CLEAN_HTML.format(credits=0, message="Kindly buy credits to continue", class="error", download_link="", disabled="disabled"))
    
    try:
        if file:
            df = pd.read_csv(file.file, low_memory=False) # Handles 5000+ rows fast
        elif text_data:
            df = pd.read_csv(io.StringIO(text_data), low_memory=False)
        else:
            return HTMLResponse(CLEAN_HTML.format(credits=credits, message="Please upload file or paste data", class="error", download_link="", disabled=""))
        ...
        download_link = f'<a href="/download?data={output.getvalue()[:10000]}" class="download">⬇️ Download Cleaned CSV</a>'
        return HTMLResponse(CLEAN_HTML.format(credits=credits, message="✅ Cleaned successfully! 1 credit used.", class="success", download_link=download_link, disabled=""))
    
    except Exception as e:
        return HTMLResponse(CLEAN_HTML.format(credits=credits, message=f"Error: {str(e)}", class="error", download_link="", disabled=""))
...
async def pay(request: Request, plan: str = Form(...), method: str = Form(...)):
    session_id = request.cookies.get("session") or str(uuid.uuid4())
    global USERS
    # HERE YOU CONNECT PAYSTACK LIVE KEY
    # For now we add credits instantly for testing
    USERS[session_id] += int(plan)
    return RedirectResponse(url="/pricing", status_code=303)

@app.get("/download")
async def download(data: str):
